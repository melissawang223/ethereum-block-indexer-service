...
type queryOption struct {
	BlockNum string
	txHash   string
	OrderBy  []string // ex: {"bank.id desc", "bank.updated_at asc"}
	Limit    int
	Offset   int
}
...
// New a row
func New(tx *gorm.DB, bankModel *models.EventLogsModel) error {
	err := tx.Table(table).
...
// GetByBankCode return a record found by bankCode
func GetByBankCode(tx *gorm.DB, bankCode string) *models.EventLogsModel {
	result := models.EventLogsModel{}
	err := tx.Table(table).
		Where("code = ?", bankCode).
		Scan(&result).Error
...
func Get(tx *gorm.DB, query *queryOption) *models.EventLogsModel {
	result := models.EventLogsModel{}
	err := tx.Table(table).
...
func Gets(tx *gorm.DB, query *queryOption) *[]models.EventLogsModel {
	result := []models.EventLogsModel{}
	err := tx.Table(table).
		Scopes(queryChain(query)).
		Scopes(limitScope(query.Limit, query.Offset)).
