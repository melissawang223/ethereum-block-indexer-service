...
// New a row
func New(tx *gorm.DB, bankModel *models.BlockModel) error {
	err := tx.Table(table).
...
func Get(tx *gorm.DB, query *queryOption) *models.BlockModel {
...
func Gets(tx *gorm.DB, query *queryOption) *[]models.BlockModel {
	result := []models.BlockModel{}
	err := tx.Table(table).
		Scopes(queryChain(query)).
		Scopes(limitScope(query.Limit, query.Offset)).
